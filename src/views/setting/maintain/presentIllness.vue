<template>
  <div class="maintain">
    ...
    <div class="manageDown">
      ...
      <a-modal centered v-model="visible" title="编辑现病史" @ok="handleOk">
...
<script>
import { getP_illnessList, editP_illnessList, deleteP_illnessList } from '@/api/setting'
...
export default {
  ...
  methods: {
    ...
    getMaintainList() {
      this.spinning = true
      getP_illnessList(this.params).then((res) => {
    ...
    handleOk(e) {
      editP_illnessList(this.form).then((res) => {
        if (res.success) {
