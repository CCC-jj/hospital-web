<template>
  <div class="charge">
    <div class="chargeTop">
      ...
      <!-- <div>
        <span style="font-size:16px;" @click="showModalReport">
          <a-icon type="file-text" theme="filled" />
          模板调用
        </span>
      </div>
      <div>
        <span style="font-size:16px;" @click="showModalSave">
    ...
    <a-modal :centered="true" v-model="caseVisible" width="1050px" title="调用模板" :footer="null">
      <div class="transferPrTop">
        <span>&nbsp;模板权限</span>
        <a-select default-value="全部" style="width: 12%">
    ...
    <div class="bottomBox">
            ...
            <a-row class="form-row" :gutter="20" type="flex" justify="center">
              ...
              <a-col>
                <a-form-model-item label="核素" prop="nuclearElement">
                  <a-input v-model="report.nuclearElement" size="large"></a-input>
                </a-form-model-item>
              ...
              <a-col>
                <a-form-model-item label="药物" prop="drug">
                  <a-input v-model="report.drug" size="large"></a-input>
                </a-form-model-item>
              </a-col>
              <a-col>
                <a-form-model-item label="剂量" prop="dosageValue">
                  <a-input v-model="report.dosageValue" size="large"></a-input>
                </a-form-model-item>
              </a-col>
              <a-col>
                <a-form-model-item label="空腹血糖" prop="bloodValue">
                  <a-input v-model="report.bloodValue" size="large"></a-input>
                </a-form-model-item>
              </a-col>
              <a-col>
                <a-form-model-item label="检查项目" prop="checkItemName">
                  <a-input v-model="report.checkItemName" size="large"></a-input>
                </a-form-model-item>
              </a-col>
            ...
            <a-row class="form-row" :gutter="20" type="flex" justify="center">
              <a-col>
                <a-form-model-item label="检查方式" prop="checkAwayName">
                  <a-input v-model="report.checkAwayName" size="large"></a-input>
                </a-form-model-item>
              </a-col>
              <a-col>
                <a-form-model-item label="检查时间" prop="checkTime">
                  <a-date-picker valueFormat="value" @change="changeCheckTime" v-model="report.checkTime" :disabled-date="disabledDate" type="date" style="width: 100%" size="large" />
                </a-form-model-item>
              </a-col>
              <a-col>
                <a-form-model-item label="申请医院" prop="applyOrgName">
                  <a-input v-model="report.applyOrgName" size="large"> </a-input>
                </a-form-model-item>
              </a-col>
              <a-col>
                <a-form-model-item label="申请科室" prop="applyDeptName">
                  <a-input v-model="report.applyDeptName" size="large"></a-input>
                </a-form-model-item>
              </a-col>
              <a-col>
                <a-form-model-item label="申请时间" prop="applyTime">
                  <a-date-picker valueFormat="value" @change="changeApplyTime" v-model="report.applyTime" :disabled-date="disabledDate" type="date" style="width: 100%" size="large" />
                </a-form-model-item>
              </a-col>
            ...
            <a-row class="form-row" :gutter="20" type="flex" justify="center">
              <a-col>
                <a-form-model-item label="申请医生" prop="applyDoctorName">
                  <a-input v-model="report.applyDoctorName" size="large"></a-input>
                </a-form-model-item>
              </a-col>
              <a-col>
                <a-form-model-item label="阴阳性" prop="yinYangResult">
                  <a-select v-model="report.yinYangResult" size="large">
              ...
              <a-col>
                <a-form-model-item label="影像质量" prop="imageQuality">
                  <a-select v-model="report.imageQuality" size="large">
              ...
              <a-col>
                <a-form-model-item label="回访状态" prop="returnVisitStatus">
                  <a-select v-model="report.returnVisitStatus" size="large">
              ...
              <a-col>
                <a-form-model-item label="报告质量" prop="reportQuality">
                  <a-select v-model="report.reportQuality" size="large">
                    <a-select-option :value="1">高</a-select-option>
                    <a-select-option :value="2">一般</a-select-option>
                    <a-select-option :value="3">较差</a-select-option>
                  </a-select>
              ...
              <!-- <a-col >
                <a-form-model-item label="开单目的" prop="orderAim">
                  <a-select v-model="form.orderAim" size="large">
              ...
              <a-col >
                <a-form-model-item label="患者来源" prop="source">
                  <a-select v-model="form.source" size="large">
      ...
      <div class="reportInfoBox">
        ...
        <div>
          ...
          <a-textarea v-model="report.checkResult" :rows="8" />
        ...
        <div v-if="auditHistory">
          ...
          <a-timeline>
            <a-timeline-item :color="timeLineColor(item.result)" v-for="(item,index) in auditHistory" :key="index">
              <p>{{item.auditTime}}</p>
              <p>审核医院：{{item.hospital}} &nbsp;&nbsp; 审核科室：{{item.department}} &nbsp;&nbsp; 审核人：{{item.doctor}}</p>
              <p>审核意见：{{item.opinion}}</p>
              <p>
                <span v-if="item.result==0">审核结果：未审核</span>
                <span v-if="item.result==1">审核结果：审核通过</span>
                <span v-if="item.result==2">审核结果：审核不通过</span>
              </p>
            </a-timeline-item>
          </a-timeline>
        </div>
        <!-- <div v-for="(item,index) in auditHistory" :key="index">
          <p class="reportInfoTitle">审核信息 {{index+1}}</p>
          <p>审核时间：{{item.auditTime}}</p>
          <p>审核科室：{{item.department}}</p>
          <p>审核人：{{item.doctor}}</p>
          <p>审核医院：{{item.hospital}}</p>
          <p>审核意见：{{item.opinion}}</p>
          <p v-if="item.result==0">审核结果：未审核</p>
          <p v-if="item.result==1">审核结果：审核通过</p>
          <p v-if="item.result==2">审核结果：审核不通过</p>
        </div> -->
        <div class="reportDiagnosis">
          ...
          <!-- <div>
            <span> 审核 &nbsp; </span>
            <a-select v-model="form.auditDoctorName">
          ...
          <!-- <div>
            <span> 审核日期 &nbsp; </span>
            <a-date-picker v-model="form.auditTime" style="width:150px;" />
          </div> -->
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    changeCheckTime(dates, dateStrings) {
      this.report.checkTime = dateStrings
    },
    changeApplyTime(dates, dateStrings) {
      this.report.applyTime = dateStrings
    },
    changeDiagnosisTime(dates, dateStrings) {
      this.report.diagnosisTime = dateStrings
    },
    disabledDate(current) {
      // Can not select days before today and today
      return current && current > moment().endOf('day')
    },
    diagnosisDisabledDate(current) {
      return (
